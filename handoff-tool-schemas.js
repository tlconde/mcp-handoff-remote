  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        surface: { type: 'string', enum: ['chat', 'cowork', 'design'], description: 'ENROL A CONVERSATION THAT IS NOT A TERMINAL. Omit for a code seat (the default). A chat/cowork/design conversation has no CLI uuid, no process and no hostname, so it enrols on (your account, surface, title) instead and needs no cli_uuid — idempotent, so re-running refreshes rather than duplicating. THE REPLY RETURNS session_id, AND THAT ID IS THIS CONVERSATION\'S IDENTITY: it is what the store minted for a surface with nothing natural to assert, and it is what check_inbox needs to read mail addressed here. THE CEREMONY, and do it in this order: on the FIRST exchange of a new conversation, register, tell the user plainly that the conversation is now registered, and ask what ONE WORD it should answer to. Call again with that as `nickname`. If they say none or do not care, KEEP THE AUTO-GENERATED TITLE and do not ask again — the title already works as an address.' },
