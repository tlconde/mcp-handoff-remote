...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Give a session on ANOTHER DEVICE a record in the shared store, so it is visible and addressable from every machine. Use when a device cannot mint its own identity: register_session needs a CLI uuid from a real local Claude Code session and refuses without one, and a device on the far side of the relay has none HERE. Also the enrolment verb when the user types "Start onboarding for <name>" or /onboard <name> on an asserted code seat (Grok): title is that word; device, session_uuid, subscription, model_slug must already be known — do not invent them. The record is honest about what it is not: identity is asserted with attested_by access (never CLI-verified), native_ref stays null until that device agent claims it, and reachability reads unknown until its agent reports in. Idempotent on (device, title): re-registering on reconnect updates rather than duplicating.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        surface: { type: 'string', enum: ['chat', 'cowork', 'design'], description: 'ENROL A CONVERSATION THAT IS NOT A TERMINAL. Omit for a code seat. Chat/cowork/design has no CLI uuid and no hostname; it enrols on (account, surface, title). Idempotent: same title refreshes. THE REPLY RETURNS session_id — that id IS this conversation. USER PHRASE, any surface: they type "Start onboarding for <name>" or /onboard <name>. Call ONCE with title:<name>, nickname:<name>, and surface only when this seat is chat/cowork/design. One word. Do not ask for another. Then whoami. Empty name → whoami only. The ask-for-a-word ceremony is retired.' },
