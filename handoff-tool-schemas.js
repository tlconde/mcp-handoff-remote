  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        surface: { type: 'string', enum: ['chat', 'cowork', 'design'], description: 'ENROL A CONVERSATION THAT IS NOT A TERMINAL. Omit for a code seat. Chat/cowork/design has no CLI uuid and no hostname; it enrols on (account, surface, title). Idempotent: same title refreshes. THE REPLY RETURNS session_id — that id IS this conversation. USER PHRASE, preferred: they type "Start onboarding for <name>" or /onboard <name>. Call ONCE with surface, title:<name>, nickname:<name>. One word. Do not ask for another. Then whoami. Empty name → whoami only, do not invent one from the folder or the generated title. The ask-for-a-word ceremony is retired.' },
