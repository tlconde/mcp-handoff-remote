...
/* ── ROUTING (pure) ───────────────────────────────────────────────────────────────────────── */
{
  ...
  withEnv({ HANDOFF_NOTIFY_LOG: logPath, HANDOFF_TEST: "1", HANDOFF_TERMINAL_NOTIFIER: null }, () => {
    const r = fresh().notify({ title: 'routing', body: 'b' });
    /* fired MUST be false: nothing appeared on anyone's screen. It used to return true, which is
     * the same lie the wake seam told on 2026-08-10 — and worse here, because the wake tier
     * branches on `fired` to decide whether it degraded to 'notify' or fell through to 'store',
     * so a notify seam could rewrite the recorded rung of a REAL delivery. */
    ok(r.fired === false && r.seam === true && r.channel === 'log-seam',
      'the log seam records instead of firing, and says so — fired:false, seam:true');
  });
